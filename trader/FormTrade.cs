...
namespace trader
{
	public partial class FormTrade : Form
	{
        ...
        private void plot()
        {
            ...
            ///////蜡烛图///////////
            double[] opens = { 2555.0, 2548.0, 2548.00, 2533.00, 2533.00, 2560.0 };
            double[] closes = { 2563.00, 2539.0, 2557.00, 2541.00, 2560.00, 2568.00 };
            double[] lows = { 2540.00, 2532.00, 2461.00, 2528.00, 2492.00, 2558.00 };
            System.Double[] highs = { 2606.00, 2554.00, 2565.00, 2546.00, 2574.00, 2594.00 };
        ...
        */
        ...
        public void FormTrade_Load(object sender, EventArgs e)
		{
            /* add strategt */
            List<MyItem> list = new List<MyItem>();
            list.Add(new MyItem("简单策略", 1));
            list.Add(new MyItem("开盘区间突破策略", 2));
            comboBox2.DataSource = list;
            comboBox2.DisplayMember = "Name";

            /* interested auto Contract*/
			...
			if (Properties.Settings.Default.Servers == null || Properties.Settings.Default.Servers.Count == 0)
			{
				...
				Properties.Settings.Default.Servers.Add("上期(模拟)|9999|tcp://180.168.146.187|10010|tcp://180.168.146.187|10000|114128");
			}
			...
			{   
				...
				//行情接收线程
				mdApi = new MdApi(tradeApi.InvestorID, ul.tbPassword.Text, Properties.Settings.Default.Servers[ul.cbServer.SelectedIndex].Split('|')[1],
					Properties.Settings.Default.Servers[ul.cbServer.SelectedIndex].Split('|')[2] + ":" + Properties.Settings.Default.Servers[ul.cbServer.SelectedIndex].Split('|')[3]);
		...
		void tradeApi_OnRspQryInstrumentCommissionRate(ref CThostFtdcInstrumentCommissionRateField pInstrumentCommissionRate, ref CThostFtdcRspInfoField pRspInfo, int nRequestID, bool bIsLast)
		{
			if (bIsLast)
			{
				if (pRspInfo.ErrorID == 0 && this.instrument4QryRate.StartsWith(pInstrumentCommissionRate.InstrumentID))
				{
					...
					{
						if (pInstrumentCommissionRate.OpenRatioByMoney == 0) //shouxufei
						{
		...
		void btnLogin_Click(object sender, EventArgs e)
		{
			...
			if (ul.cbServer.SelectedIndex >= 0)
			{
				...
				if (server.Length >= 8)
				{
					...
					tradeApi = new TradeApi(ul.tbUserID.Text, ul.tbPassword.Text, server[1], server[4] + ":" + server[5]);
					regEvents();
		...
		void progress(EnumProgessState _state, string _msg)
		{
            ...
            /*
			if (_state == EnumProgessState.OnConnected)		
			{
		...
		void subAllInstruments()
		{
            ...
            //合约列表
            Console.WriteLine("[subAllInstruments] dtInstruments.Rows.Count = " + dtInstruments.Rows.Count);
			for (int i = 0; i < this.dtInstruments.Rows.Count; i++)
			{
				...
				mdApi.SubMarketData(instrument);					//订阅行情
				//自定义手数:默认为1
        ...
        void mdApi_OnRtnDepthMarketData(ref CThostFtdcDepthMarketDataField pDepthMarketData)
		{
            ...
            strategy.ITick(this, ref pDepthMarketData);
            //Console.WriteLine(pDepthMarketData.HighestPrice);
            if (pDepthMarketData.HighestPrice != double.MinValue && pDepthMarketData.HighestPrice != double.MaxValue)
            {
                if ((string)ht[pDepthMarketData.InstrumentID] == "1") {
                    return;
                }
                ht[pDepthMarketData.InstrumentID] = "1";

                string timenow = DateTime.Now.ToLongTimeString().ToString();
                string[] timearr = timenow.Split(':');
                const int AFTERN = 915 + 5;
                int now = Convert.ToInt32(timearr[0]) * 60 + Convert.ToInt32(timearr[1]);
                //Console.WriteLine(timenow + "  " + now + "   " + AFTERN);
                if (now >= AFTERN)
                {
                    string todaydata = DateTime.Now.ToString("yyyy/MM/dd"); ;
                    dbConn.AddMarketInfo(pDepthMarketData.InstrumentID, todaydata, pDepthMarketData.OpenPrice, pDepthMarketData.HighestPrice
                        , pDepthMarketData.LowestPrice, pDepthMarketData.ClosePrice);
		...
		void tradeApi_OnRspUserLogin(ref CThostFtdcRspUserLoginField pRspUserLogin, ref CThostFtdcRspInfoField pRspInfo, int nRequestID, bool bIsLast)
		{
			if (bIsLast)
			{
				...
				else
				{
                    ...
                    {
                        Console.WriteLine("Read   Resources\\rate" + "_" + tradeApi.InvestorID + ".xml");
                        this.dtInstruments.ReadXml("Resources\\rate" + "_" + tradeApi.InvestorID + ".xml"); //读取合约信息
                    }
                    Console.WriteLine("[tradeApi_OnRspUserLogin] dtInstruments.Rows.Count = " + dtInstruments.Rows.Count);
        ...
        */
		...
		void freshAccount(CThostFtdcTradingAccountField pTradingAccount)
		{
            ...
            dbConn.AddAcountInfo(pTradingAccount.BrokerID, pTradingAccount.AccountID, 
                pTradingAccount.CloseProfit, pTradingAccount.PositionProfit, pTradingAccount.Balance, DateTime.Now.ToShortDateString().ToString());
		...
		{
			if (pRspInfo.ErrorID != 0)
				this.BeginInvoke(new Action<EnumProgessState, string>(progress),EnumProgessState.OrderInsert, pRspInfo.ErrorMsg);
		}
		...
		void tradeApi_OnRtnTrade(ref CThostFtdcTradeField pTrade)
		{
            ...
            dbConn.AddTrader(pTrade.InvestorID, pTrade.InstrumentID, pTrade.TradeTime, pTrade.OrderRef, pTrade.TradeID, pTrade.Price, pTrade.Volume
                ,pTrade.OffsetFlag, pTrade.Direction, pTrade);
            if (pTrade.Direction == EnumDirectionType.Sell) {
                dbConn.AddTraderResult(pTrade);
            }
        ...
        */
		...
		{
            //foreach (string _name in (new string[] { "SHFE", "CZCE", "DCE", "CFFEX", "Selected"}))
		...
		void dgvMD_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
		{
			try
			{
				if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
				{
                    ...
                    this.button5.Text = (string)(sender as DoubleBufferDGV).Rows[e.RowIndex].Cells["合约"].Value;
                    ...
                    } else if (dgv.Columns[e.ColumnIndex].Name == "合约"){
                        label_symbol.Text = instrument;

                        DataStock = dbConn.QueryMarketInfo(instrument, 300);
                        if (DataStock.Count == 0) { return; }
        ...
        private void plotSurface2D1_Click(object sender, EventArgs e)
        {
        ...
        */
        ...
        {
            /*
            file = "C:\\Users\\Jack\\Desktop\\trader\\KLine\\rb1609_d1.txt";
            FormCtp_Load();
            */
        }

        //public bool OpenAutoTrade = false;
        private void button1_Click(object sender, EventArgs e)
        {
            if (strategy.AutoTrade == 0) {
                string instrument = button5.Text;
                string winstop = textBox1.Text;
                string losestop = textBox2.Text;
                MyItem item = comboBox2.SelectedItem as MyItem;
                int strategyid = item.Value;

                int winstopnum, lossstopnum;
                try
                {
                    winstopnum = Convert.ToInt32(winstop);
                    lossstopnum = Convert.ToInt32(losestop);

                    strategy.StartAutoTrade(instrument, winstopnum, lossstopnum, strategyid);
                    strategy.AutoTrade = 1;
                    this.button1.Text = "关闭自动化交易";
                }
                catch (Exception eee) {
        ...
        private void labelVolumeMax_Click(object sender, EventArgs e)
        {
        ...
        private void comboAutoInstrument_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private void dataGridViewCZCE_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
